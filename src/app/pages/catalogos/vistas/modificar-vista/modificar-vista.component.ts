...
@Component({
  selector: 'app-modificar-vista',
  templateUrl: './modificar-vista.component.html'
})
export class ModificarVistaComponent implements OnInit {
  vista: Vista = new Vista();

  constructor(private VistaS: VistaService, private route: ActivatedRoute, private router: Router) { }
  ...
  ngOnInit() {
    this.route.params.subscribe(param => {
      this.vista.vistaId = parseInt(param.id, 0);
      this.VistaS.getBtyId(this.vista.vistaId).subscribe((res: any) => {
        this.vista = res.data;
       });
    });
  }
